...
function Cards(){
    ...
    <iframe src={props.src} className="w-full h-[24.8rem] rounded-md mt-4" loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe>
    ...
    <div className="absolute bottom-6 -right-4 z-20">
        <p className='w-full text-right pr-10 text-[1.8rem] sm:text-4xl tracking-wide font-staat z-10'>{props.hours1}</p>
        <p className='w-full text-right pr-10 text-[1.8rem] sm:text-4xl tracking-wide font-staat z-10 -mt-1 md:mt-2'>{props.hours2}</p>
    </div>

    return(
        <div className='flex justify-center items-center my-8 mx-4'>
            <div className='max-w-6xl w-screen h-auto grid grid-cols-1 lg:grid-cols-2 gap-6'>
                {locations.map((item, i) =>   
                    <div key={i} className='relative bg-amber-300 h-auto p-4 border-2 border-amber-300 shadow-lg rounded-lg max-w-xl mx-auto w-full md:w-full overflow-hidden'>
                        <p className='relative text-3xl sm:text-4xl font-staat z-10'>{item.location}</p>
                        <p className='relative text-xl sm:text-2xl font-semibold font-slab pt-1 z-10'>{item.number}</p>
